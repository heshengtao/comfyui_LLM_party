...
3. only_api라는 리포지토리 브랜치가 새로 추가되었으며, 이 브랜치에는 API 호출 부분만 포함되어 있어 API 호출만 필요한 사용자에게 매우 편리합니다. `comfyui`의 `custom_nodes` 폴더에서 `git clone -b only_api https://github.com/heshengtao/comfyui_LLM_party.git` 명령어를 사용하신 후, 본 프로젝트의 홈페이지에 따라 환경을 구축하시면 이 브랜치를 사용할 수 있습니다. 주의! `custom_nodes` 폴더에 `comfyui_LLM_party`라는 다른 폴더가 존재하지 않도록 확인해 주시기 바랍니다.
...
## 제 다른 유용한 오픈 소스 프로젝트:
[super-agent-party](https://github.com/heshengtao/super-agent-party)는 무한한 가능성을 가진 3D AI 데스크톱 동반자 애플리케이션입니다! 이미 RAG, 웹 검색, 장기 기억, 코드 인터프리터, MCP, A2A, Comfyui, QQ 봇 등 다양한 기능을 지원하고 있습니다!
