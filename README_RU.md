...
1. На основе [browser-use](https://github.com/gregpr07/browser-use) был разработан новый узел браузерного инструмента, который позволяет LLM автоматически выполнять браузерные задачи, которые вы публикуете.
1. Узлы для загрузки файлов, загрузки папок, загрузки веб-контента и все связанные с векторными представлениями узлы были обновлены. Теперь загружаемый вами контент файла всегда будет включать имя файла и индекс абзаца. Узел загрузки папки может фильтровать файлы, которые вы хотите загрузить, с помощью `related_characters`.
1. Добавлен инструмент локальной модели для преобразования речи в текст, который теоретически совместим со всеми ASR моделями на HF. Например: [openai/whisper-small](https://huggingface.co/openai/whisper-small), [nyrahealth/CrisperWhisper](https://huggingface.co/nyrahealth/CrisperWhisper) и т.д.
1. Добавлены узлы ASR и TTS для [fish audio](https://docs.fish.audio/introduction), пожалуйста, ознакомьтесь с документацией API fish audio для инструкций по использованию.
1. Добавлен узел загрузчика aisuite, совместимый со всеми API, которые может поддерживать aisuite, включая: ["openai", "anthropic", "aws", "azure", "vertex", "huggingface"]. Пример рабочего процесса: [start_with_aisuite](workflow/start_with_aisuite.json).
1. Добавлена новая категория: узлы памяти, которые могут использоваться для управления историей разговоров вашего LLM. В настоящее время узлы памяти поддерживают три режима для управления историей разговоров: локальные JSON-файлы, Redis и SQL. Разделяя историю разговоров LLM и сам LLM, вы можете использовать модели векторного представления слов для сжатия и организации вашей истории разговоров, что позволяет сэкономить токены и контекстные окна для LLM. Пример рабочего процесса: [Внешняя память](workflow/外置记忆.json).
