...
def get_accuweather(city,extensions="all"):
    ...
    #查询城市位置Key
    api = FreeApi('http://dataservice.accuweather.com/locations/v1/cities/search?apikey='+accuweather_key_load+'&q='+city+'&language=zh-CN&details=false')
    result = api.get_result()
    #根据key查天气
    if result:
        data = json.loads(result)
        ...
        #查询实时天气
        if extensions=="all":
            api = FreeApi('http://dataservice.accuweather.com/currentconditions/v1/'+city_key+'?apikey='+accuweather_key_load+'&language=zh-CN&details=false')
        #查询未来天气
        if extensions=="base":
            api = FreeApi('http://dataservice.accuweather.com/forecasts/v1/daily/5day/'+city_key+'?apikey='+accuweather_key_load+'&language=zh-CN&details=false')
    #返回天气信息
    result = api.get_result()
    if result:
        if extensions=="all":
