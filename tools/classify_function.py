...
class classify_function_plus:
    ...
    RETURN_NAMES = ("response1","response2","response3","response4","response5","response6","response7","response8","response9","response10",)
    ...
    def condition(self,text,is_enable=True):
        ...
        # 根据分割后的部分数量，赋值给response变量
        response1, response2, response3,response4,response5, response6, response7,response8 , response9,response10= (parts + ["", "", ""])[:10]
        if response1.strip() == "" or response1.strip() == "empty":
            response1 = None
        if response2.strip() == "" or response2.strip() == "empty":
            response2 = None
        ...
        if response10.strip() == "" or response10.strip() == "empty":
            response10 = None
        return (response1,response2,response3,response4,response5,response6,response7,response8,response9,response10,)