        ...
        for root, dirs, files in os.walk(folder_path):
            if root == folder_path:
                for file in files:
                    ...
                    if (
                        file_type == "all"
                        or file_type == "all_files"
                        or (file_type != "all_files" and ext in extensions)
                    ):
                        os.remove(file_path)
...
if __name__ == "__main__":
    obj = FolderCleaner()
    obj.delete_files("E:\ComfyUI\custom_nodes\comfyui_LLM_party\custom_tool\imgs", file_type="all", is_enable=True)
