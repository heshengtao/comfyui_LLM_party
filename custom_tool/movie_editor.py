...
class Image2Video:
    ...
    def image_to_video(
        self, audio_path, image_path, subtitle, subtitle_size, font_path, r, g, b, output_path, is_enable
    ):
        if not is_enable:
            return (None, None)
        ...
        total_text_height = (
            sum([line_height + line_spacing for line in lines[:-1]]) + line_height
        )  # 最后一行不需要额外的间距
...
class CombineVideos:
    ...
    def concatenate_videos(self, input_folder, output_folder, is_enable):
        ...
        video_files = [
            os.path.join(input_folder, f) for f in os.listdir(input_folder) if f.endswith((".mp4", ".avi", ".mov"))
        ]
...
NODE_CLASS_MAPPINGS = {"Image2Video": Image2Video, "CombineVideos": CombineVideos}
...
if language == "zh_CN" or language == "en_US":
    lang = language
if lang == "zh_CN":
    NODE_DISPLAY_NAME_MAPPINGS = {"Image2Video": "静态图片转视频🐶", "CombineVideos": "合并视频🐶"}
else:
    NODE_DISPLAY_NAME_MAPPINGS = {"Image2Video": "Image2Video🐶", "CombineVideos": "CombineVideos🐶"}
...
if __name__ == "__main__":
    ...
    obj = Image2Video()
    obj.image_to_video(audio_path, image_path, subtitle, subtitle_size, font_path, r, g, b, output_path, True)
