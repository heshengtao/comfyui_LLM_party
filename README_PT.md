...
1. Os nós do carregador LLM local foram significativamente ajustados, então você não precisa mais escolher o tipo de modelo por conta própria. O nó do carregador llava e o nó do carregador GGUF foram readicionados. O tipo de modelo no nó da cadeia de modelos LLM local foi alterado para LLM, VLM e GGUF, correspondendo ao carregamento direto de modelos LLM, carregamento de modelos VLM e carregamento de modelos LLM no formato GGUF. Agora, os modelos VLM e os modelos LLM no formato GGUF são novamente suportados. As chamadas locais agora podem ser compatíveis com mais modelos! Exemplos de fluxos de trabalho: [LLM_local](workflow/start_with_LLM_local.json), [llava](workflow/start_with_llava.json), [GGUF](workflow/start_with_GGUF.json)
