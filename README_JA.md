...
3. only_apiというリポジトリのブランチが新たに追加され、このブランチにはAPI呼び出しの部分のみが含まれており、API呼び出しのみを必要とするユーザーに便利です。`comfyui`の`custom node`フォルダで、次のコマンドを使用して`git clone -b only_api https://github.com/heshengtao/comfyui_LLM_party.git`を実行してください。そして、本プロジェクトのホームページに従って環境を構築すれば、このブランチを使用できます。注意！`custom node`フォルダ内に`comfyui_LLM_party`という名前の他のフォルダが存在しないことを確認する必要があります。
...
## 私の別の有用なオープンソースプロジェクト：
[super-agent-party](https://github.com/heshengtao/super-agent-party)は無限の可能性を秘めた3D AIデスクトップコンパニオンアプリです！RAG、ウェブ検索、長期記憶、コードインタープリター、MCP、A2A、Comfyui、QQボットなど、さまざまな機能をすでにサポートしています！
