...
3. 新增了only_api的仓库分支，该分支只包含调用API的部分，方便只需要API调用的用户，只需要在`comfyui`的`custom_nodes`文件夹中使用命令行`git clone -b only_api https://github.com/heshengtao/comfyui_LLM_party.git`，然后按照本项目主页的环境部署方案，即可使用这个分支。注意！如果你需要保证`custom_nodes`文件夹中没有其他叫`comfyui_LLM_party`的文件夹。
